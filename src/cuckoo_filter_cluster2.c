...
/*A bucket can hold a  max of 8 entries/elements (elements contain 1 finger-print(8-bit) each.*/
#define __MAX_ELEMS_IN_A_BUCKET 8

#define __MAX_CUCKOO_KICKS 5

#define TOTAL_NO_OF_BUCKETS 3  //supposed to be 16384 (total no. of slots)
...
struct __bucket
{
    //The internal m_bit_array that contains the finger-print of the given keys. 
   FINGER_PRINT_TYPE * __m_bit_finger_print_array;

   M_BIT_ARRAY_LENGTH_TYPE __m_bit_arr_len_in_bytes;   //length of the bucket (for now we can assume it to be 8)

   /*int slot_number;
   char keyHash[3];*/
};

/*const char *hash_names[3];
hash_names[0] = "abc";
hash_names[1] = "def";
hash_names[2] = "ghi";*/
...
const int slot_numbers[3] = {7638, 16148, 8421};

struct __bucket* bucket_array[3];


/*List of functions */

FINGER_PRINT_TYPE __get_pearson_8_bit_finger_print_cluster2(unsigned const char *key);
FINGER_PRINT_TYPE __get_finger_print_cluster2(const char* key);

M_BIT_ARRAY_LENGTH_TYPE __murmur3_32_bit_hash_cluster2(const char *key);
M_BIT_ARRAY_LENGTH_TYPE __get_hash_1_cluster2(const char* key);
M_BIT_ARRAY_LENGTH_TYPE __get_hash_2_cluster2(FINGER_PRINT_TYPE finger_print, M_BIT_ARRAY_LENGTH_TYPE hash1);

short __insert_element_cluster2(client* c, const char* key);
short __check_or_delete_element_cluster2(struct __cuckoo_filter* filter, const char* key, short __should_delete_element);
short __is_element_present_in_bucket_cluster2(FINGER_PRINT_TYPE finger_print,M_BIT_ARRAY_LENGTH_TYPE bucket_no,const struct __cuckoo_filter* const my_filter,  M_BIT_ARRAY_LENGTH_TYPE * indexWhereElemIsPresent);
short __remove_element_cluster2(struct __cuckoo_filter* filter, const char* key);
short __remove_specified_filter_cluster2(struct __cuckoo_filter* temp);

M_BIT_ARRAY_LENGTH_TYPE __getBucketNumber_cluster2(M_BIT_ARRAY_LENGTH_TYPE elem_insertion_index);
M_BIT_ARRAY_LENGTH_TYPE __getFirstIndexOfTheBucket_cluster2(const struct __bucket * const buck);
M_BIT_ARRAY_LENGTH_TYPE __getLastIndexOfTheBucket_cluster2(const struct __bucket * const buck);
short __is_bucket_empty_cluster2(const struct __bucket *my_bucket, M_BIT_ARRAY_LENGTH_TYPE * indexOfEmptyElement);

/*short add_element_cluster2(struct __cuckoo_filter* filter,const char* key);
short is_member_cluster2(struct __cuckoo_filter* filter, const char* key);
short delete_element_cluster2(struct __cuckoo_filter* filter, const char* key);*/
...
*/
/*short
__is_element_present_in_bucket_cluster2(FINGER_PRINT_TYPE finger_print,M_BIT_ARRAY_LENGTH_TYPE bucket_no,const struct __cuckoo_filter* const my_filter, M_BIT_ARRAY_LENGTH_TYPE * indexWhereElemIsPresent)
{
    ...
    * indexWhereElemIsPresent = 0;
    ...
    M_BIT_ARRAY_LENGTH_TYPE firstIndex = __getFirstIndexOfTheBucket_cluster2(my_filter, bucket_no);

    M_BIT_ARRAY_LENGTH_TYPE lastIndex = __getLastIndexOfTheBucket_cluster2(my_filter, bucket_no);
...
__getBucketNumber_cluster2(M_BIT_ARRAY_LENGTH_TYPE elem_insertion_index)
{
//#if PRINT_TRACE
...
//#endif
   ...
   assert(bucketNo >= 1 && bucketNo <= TOTAL_NO_OF_BUCKETS);
   return bucketNo;
}
...
*/
M_BIT_ARRAY_LENGTH_TYPE
__getFirstIndexOfTheBucket_cluster2(const struct __bucket * const buck)
{
//#if PRINT_TRACE
...
//#endif
    ...
    firstIndex = 0;
    assert(firstIndex >= 0 && firstIndex <= buck->__m_bit_arr_len_in_bytes - 1);
    serverLog(LL_VERBOSE,"first index of bucket %d is %d", firstIndex);
...
*/
M_BIT_ARRAY_LENGTH_TYPE
__getLastIndexOfTheBucket_cluster2(const struct __bucket * const buck)
{
// #if PRINT_TRACE
...
//#endif   assert(bucket_no >= 1);
    ...
    if(lastIndex >= buck->__m_bit_arr_len_in_bytes)
        lastIndex = buck->__m_bit_arr_len_in_bytes - 1;

    assert(lastIndex >= 0 && lastIndex <= buck->__m_bit_arr_len_in_bytes - 1);

    serverLog(LL_VERBOSE,"last index of bucket %d is %d", lastIndex);
...
FINGER_PRINT_TYPE
__get_pearson_8_bit_finger_print_cluster2(unsigned const char *key)
{
//#if PRINT_TRACE
...
M_BIT_ARRAY_LENGTH_TYPE
__murmur3_32_bit_hash_cluster2(const char* key)
{
// #if PRINT_TRACE
...
*/
short
__is_bucket_empty_cluster2(const struct  __bucket *my_bucket, M_BIT_ARRAY_LENGTH_TYPE * indexOfEmptyElement)
{
// #if PRINT_TRACE
...
//#endif
    ...
    //get starting index of bucket in the given filter...
    M_BIT_ARRAY_LENGTH_TYPE firstIndex = __getFirstIndexOfTheBucket_cluster2(my_bucket);

    M_BIT_ARRAY_LENGTH_TYPE lastIndex = __getLastIndexOfTheBucket_cluster2(my_bucket);
    //get ending index of bucket in the given filter.
    ...
    for(iterIndex = firstIndex; iterIndex <= lastIndex; iterIndex ++)
    {
        if(__EMPTY_ELEMENT == my_bucket->__m_bit_finger_print_array[iterIndex])
        {
...
//#if PRINT_DEBUG
            //printf("\n The filter named '%s' has an empty free in the bucket number '%d'\n",my_filter->__name,bucket_no);
//#endif
            //serverLog(LL_VERBOSE,"The bucket has an empty space at the bucket number %d",bucket_no);
...
*/
//M_BIT_ARRAY_CONTENT_TYPE __get_finger_print_cluster2(int key)
FINGER_PRINT_TYPE
__get_finger_print_cluster2(const char* key)
{
// #if PRINT_TRACE
...
//#endif //User pearsons hashing technique for generating the 8-bit fingerprint for given key.
  ...
  if(NULL == key)
    return 0;
  return __get_pearson_8_bit_finger_print_cluster2(key);
...
*/
M_BIT_ARRAY_LENGTH_TYPE
__get_hash_1_cluster2(const char* key)
{
//#if PRINT_TRACE
...
//#endif
    ...
    if(NULL == key)
        return 0;
    return __murmur3_32_bit_hash_cluster2(key);
}
...
*/
M_BIT_ARRAY_LENGTH_TYPE
__get_hash_2_cluster2(FINGER_PRINT_TYPE finger_print, M_BIT_ARRAY_LENGTH_TYPE hash1)
{
// #if PRINT_TRACE
    //printf("\n [PRINT_TRACE] Getting Second Hash\n");
//#endif
     serverLog(LL_NOTICE,"Getting Second Hash");
    //return hash1 XOR __get_hash_1_cluster2(finger_print)
    return (hash1 ^ __get_hash_1_cluster2(&finger_print));
}
...
*/
short
__insert_element_cluster2(client* c, const char* key)
{
    ...
    //assert(bucket->__m_bit_finger_print_array);

    //get finger print for given key..
    FINGER_PRINT_TYPE finger_print = __get_finger_print_cluster2(key);

    M_BIT_ARRAY_LENGTH_TYPE hash1 = __get_hash_1_cluster2(key);
    ...
    //hash1 = hash1 % (bucket->__m_bit_arr_len_in_bytes * TOTAL_NO_OF_BUCKETS);   //total length of filter
    
    hash1 = hash1 % (24);  //24 is the total length of the filter
    M_BIT_ARRAY_LENGTH_TYPE bucket_1 = __getBucketNumber_cluster2(hash1);        //gives slot no. 1

    

    //char* __bucket_name1 = hash_names[bucket_1];
    name = createObject(OBJ_STRING, (void *)hash_names[bucket_1]); 
    ...
    if (len < sizeof(struct __bucket)) {
        addReplyError(c,err);
        return;
    }

    struct __bucket* bucket1 = (struct __bucket*)o->ptr;
    ...
    if(__is_bucket_empty_cluster2(bucket1, &emptyLocationInBucket))
    {
        //Fill this entry with the fingerprint.
        bucket1->__m_bit_finger_print_array[emptyLocationInBucket] = finger_print;
//#if PRINT_DEBUG
...
//#endif
    ...
    //M_BIT_ARRAY_LENGTH_TYPE hash2 = __get_hash_2_cluster2(finger_print,hash1);
    M_BIT_ARRAY_LENGTH_TYPE hash2 = __get_hash_2_cluster2(finger_print, bucket_1);

    //This hash2 must be < finger_print array length.
    //hash2 = hash2 % filter->__m_bit_arr_len_in_bytes;
    hash2 = hash2 % 24;

    M_BIT_ARRAY_LENGTH_TYPE bucket_2 = __getBucketNumber_cluster2(hash2);
    

    //char* __bucket_name2 = hash_names[bucket_2];
    name = createObject(OBJ_STRING, (void *)hash_names[bucket_2]); 
    ...
    struct __bucket* bucket2 = (struct __bucket*)o->ptr;


    if(__is_bucket_empty_cluster2(bucket2, &emptyLocationInBucket))
    {
        //Fill this entry with the fingerprint.
        bucket2->__m_bit_finger_print_array[emptyLocationInBucket] = finger_print;
//#if PRINT_DEBUG
...
//#endif
        ...
        name = createObject(OBJ_STRING, (void *)hash_names[*(randomlyChosenBucket)]); 
        ...
        struct __bucket* randomBucket = (struct __bucket*)o->ptr;



        //assert(randomlyChosenBucket >= 1 && randomlyChosenBucket <= 2);
        //firstIndex = __getFirstIndexOfTheBucket_cluster2(filter,*(randomlyChosenBucket));
        //lastIndex = __getLastIndexOfTheBucket_cluster2(filter,*(randomlyChosenBucket));
        ...
        for(cuckoo_kick_count = 1; cuckoo_kick_count <= __MAX_CUCKOO_KICKS; cuckoo_kick_count++)
        {
            ...
            actualElemsInBucket = __getLastIndexOfTheBucket_cluster2(randomBucket) - __getFirstIndexOfTheBucket_cluster2(randomBucket) ;
            ...
            randomlySelectedIndex = (randomNumber);
            ...
            serverLog(LL_VERBOSE,"Swapping new finger-print %c and selected finger-print %c",finger_print,randomBucket->__m_bit_finger_print_array[randomlySelectedIndex]);
            tempFingerPrint = randomBucket->__m_bit_finger_print_array[randomlySelectedIndex];
            randomBucket->__m_bit_finger_print_array[randomlySelectedIndex] = finger_print;
            ...
            hash2 = __get_hash_2_cluster2(finger_print,*(randomlyChosenBucket));
            //This hash2 must be < finger_print array length.
            //hash2 = hash2 % filter->__m_bit_arr_len_in_bytes;
            hash2 = hash2 % 24;
            ...
            bucket_2 = __getBucketNumber_cluster2(hash2);
            ...
            randomBucket = (struct __bucket*)o->ptr;

            if(__is_bucket_empty_cluster2(randomBucket, &emptyLocationInBucket))
            {
                //Fill this entry with the fingerprint.
                randomBucket->__m_bit_finger_print_array[emptyLocationInBucket] = finger_print;
...
/*
short
__check_or_delete_element_cluster2(struct __cuckoo_filter* filter , const char* key, short __should_delete_element)
{
    ...
    M_BIT_ARRAY_LENGTH_TYPE bucket_1 = __getBucketNumber_cluster2(filter,hash1);
    M_BIT_ARRAY_LENGTH_TYPE locWhereElemIsPresent = 0;

    //Check if element is present in first bucket.
    if(__is_element_present_in_bucket_cluster2(finger_print,bucket_1,filter,&locWhereElemIsPresent))
    {
    ...
    if(__is_element_present_in_bucket_cluster2(finger_print,bucket_2,filter,&locWhereElemIsPresent))
    {
...
/*
short
__remove_element_cluster2(struct __cuckoo_filter* filter , const char* key)
{
    serverLog(LL_NOTICE,"\"Removing the element\"");
    if(NULL == key)
        return 0;
    return __check_or_delete_element_cluster2(filter,key,1);
}
...
/*
short
__remove_specified_filter_cluster2(struct __cuckoo_filter* temp)
{
...
/*
short
add_element_cluster2(struct __cuckoo_filter* filter,const char* key)
{
    serverLog(LL_NOTICE,"Adding element to cuckoo filter");
    if(NULL == key)
        return 0;
    return __insert_element_cluster2(key);
}
...
/*
    ...
    Output :
        ...
        Returns 1 -- if the key is found in the cuckoo filter 'name'
*/
//unsigned short is_member_cluster2(const char* name, const int key)

/*
short
is_member_cluster2(struct __cuckoo_filter* filter, const char* key)
{
    serverLog(LL_NOTICE,"Checking if element is member of filter.");
    if( NULL == key)
        return 0;

    return __check_or_delete_element_cluster2(filter,key,0);
}
...
/*
short
delete_element_cluster2(struct __cuckoo_filter* filter, const char* key)
{
    serverLog(LL_NOTICE,"Deleting element from filter");
    if(NULL == key)
        return 0;

    return __remove_element_cluster2(filter,key);
}
*/
...
void
cuckoocreatecluster2Command(client *c)
{
    robj *o;
    long m;
    size_t byte;
    ...
    */
    ...
    for(i=0; i<3; i++)
    {
        robj* key = (robj*) hash_names[i];
        o = createObject(OBJ_STRING, sdsnewlen(NULL, byte));
        dbAdd(c->db, key, o);

        new_bucket = (struct __bucket*)o->ptr;
        new_bucket->__m_bit_arr_len_in_bytes = 8;
        new_bucket->__m_bit_finger_print_array = (FINGER_PRINT_TYPE*)(new_bucket+1);

        bucket_array[i] = new_bucket;
    }
}
...
void cuckooinsertcluster2Command(client *c) {
    ...
    */
    ...
    struct __bucket* bucket_;
    
    for (i = 2; i < c->argc; i++) {
        //all the arguments are considered as robj's internally.
        //So instead of sending c->argv[i], send (c->argv[i])->ptr
        //if(__insert_element_cluster2(filter,c->argv[i]))
        if(__insert_element_cluster2(c, (c->argv[i])->ptr))
...
void cuckooremovecluster2Command(client *c) {
    ...
    for (i = 2; i < c->argc; i++) {
        //all the arguments are considered as robj's internally.
        //So instead of sending c->argv[i], send (c->argv[i])->ptr
        //if(delete_element_cluster2(filter,c->argv[i]))
        if(delete_element_cluster2(filter,(c->argv[i])->ptr))
            addReply(c,shared.cone);
        else
            addReply(c,shared.czero);
    }
...
void cuckoocheckcluster2Command(client *c) {
    ...
    for (i = 2; i < c->argc; i++) {
        //all the arguments are considered as robj's internally.
        //So instead of sending c->argv[i], send (c->argv[i])->ptr
        //if(is_member_cluster2(filter,c->argv[i]))
        if(is_member_cluster2(filter,(c->argv[i])->ptr))
            addReply(c,shared.cone);
        else
            addReply(c,shared.czero);
    }
