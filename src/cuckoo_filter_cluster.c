...
FINGER_PRINT_TYPE __get_pearson_8_bit_finger_print_cluster(unsigned const char *key);
FINGER_PRINT_TYPE __get_finger_print_cluster(const char* key);

M_BIT_ARRAY_LENGTH_TYPE __murmur3_32_bit_hash_cluster(const char *key);
M_BIT_ARRAY_LENGTH_TYPE __get_hash_1_cluster(const char* key);
M_BIT_ARRAY_LENGTH_TYPE __get_hash_2_cluster(FINGER_PRINT_TYPE finger_print, M_BIT_ARRAY_LENGTH_TYPE hash1);

short __insert_element_cluster(struct __cuckoo_filter* filter,const char* key);
short __check_or_delete_element_cluster(struct __cuckoo_filter* filter, const char* key, short __should_delete_element);
short __is_element_present_in_bucket_cluster(FINGER_PRINT_TYPE finger_print,M_BIT_ARRAY_LENGTH_TYPE bucket_no,const struct __cuckoo_filter* const my_filter,  M_BIT_ARRAY_LENGTH_TYPE * indexWhereElemIsPresent);
short __remove_element_cluster(struct __cuckoo_filter* filter, const char* key);
short __remove_specified_filter_cluster(struct __cuckoo_filter* temp);

M_BIT_ARRAY_LENGTH_TYPE __getBucketNumber_cluster(const struct __cuckoo_filter * const ckFilter, M_BIT_ARRAY_LENGTH_TYPE elem_insertion_index);
M_BIT_ARRAY_LENGTH_TYPE __getFirstIndexOfTheBucket_cluster(const struct __cuckoo_filter * const ckFilter, M_BIT_ARRAY_LENGTH_TYPE bucket_no);
M_BIT_ARRAY_LENGTH_TYPE __getLastIndexOfTheBucket_cluster(const struct __cuckoo_filter * const ckFilter, M_BIT_ARRAY_LENGTH_TYPE bucket_no);
short __is_bucket_empty_cluster(const struct __cuckoo_filter *my_filter, const M_BIT_ARRAY_LENGTH_TYPE index,M_BIT_ARRAY_LENGTH_TYPE * indexOfEmptyElement);

short add_element_cluster(struct __cuckoo_filter* filter,const char* key);
short is_member_cluster(struct __cuckoo_filter* filter, const char* key);
short delete_element_cluster(struct __cuckoo_filter* filter, const char* key);
...
*/
short
__is_element_present_in_bucket_cluster(FINGER_PRINT_TYPE finger_print,M_BIT_ARRAY_LENGTH_TYPE bucket_no,const struct __cuckoo_filter* const my_filter, M_BIT_ARRAY_LENGTH_TYPE * indexWhereElemIsPresent)
{
    ...
    * indexWhereElemIsPresent = 0;
    ...
    M_BIT_ARRAY_LENGTH_TYPE firstIndex = __getFirstIndexOfTheBucket_cluster(my_filter, bucket_no);

    M_BIT_ARRAY_LENGTH_TYPE lastIndex = __getLastIndexOfTheBucket_cluster(my_filter, bucket_no);
...
__getBucketNumber_cluster(const struct __cuckoo_filter* const ckFilter, M_BIT_ARRAY_LENGTH_TYPE elem_insertion_index)
{
//#if PRINT_TRACE
...
*/
M_BIT_ARRAY_LENGTH_TYPE
__getFirstIndexOfTheBucket_cluster(const struct __cuckoo_filter * const ckFilter , M_BIT_ARRAY_LENGTH_TYPE bucket_no)
{
//#if PRINT_TRACE
...
*/
M_BIT_ARRAY_LENGTH_TYPE
__getLastIndexOfTheBucket_cluster(const struct __cuckoo_filter * const ckFilter, M_BIT_ARRAY_LENGTH_TYPE bucket_no)
{
// #if PRINT_TRACE
...
FINGER_PRINT_TYPE
__get_pearson_8_bit_finger_print_cluster(unsigned const char *key)
{
//#if PRINT_TRACE
...
M_BIT_ARRAY_LENGTH_TYPE
__murmur3_32_bit_hash_cluster(const char* key)
{
// #if PRINT_TRACE
...
*/
short
__is_bucket_empty_cluster(const struct  __cuckoo_filter *my_filter, const M_BIT_ARRAY_LENGTH_TYPE bucket_no, M_BIT_ARRAY_LENGTH_TYPE * indexOfEmptyElement)
{
// #if PRINT_TRACE
...
*/
//M_BIT_ARRAY_CONTENT_TYPE __get_finger_print_cluster(int key)
FINGER_PRINT_TYPE
__get_finger_print_cluster(const char* key)
{
// #if PRINT_TRACE
...
*/
M_BIT_ARRAY_LENGTH_TYPE
__get_hash_1_cluster(const char* key)
{
//#if PRINT_TRACE
...
*/
M_BIT_ARRAY_LENGTH_TYPE
__get_hash_2_cluster(FINGER_PRINT_TYPE finger_print, M_BIT_ARRAY_LENGTH_TYPE hash1)
{
// #if PRINT_TRACE
    //printf("\n [PRINT_TRACE] Getting Second Hash\n");
//#endif
     serverLog(LL_NOTICE,"Getting Second Hash");
    //return hash1 XOR __get_hash_1_cluster(finger_print)
    return (hash1 ^ __get_hash_1_cluster(&finger_print));
}
...
*/
short
__insert_element_cluster(struct __cuckoo_filter* filter,const char* key)
{
    ...
    FINGER_PRINT_TYPE finger_print = __get_finger_print_cluster(key);

    M_BIT_ARRAY_LENGTH_TYPE hash1 = __get_hash_1_cluster(key);
    ...
    M_BIT_ARRAY_LENGTH_TYPE bucket_1 = __getBucketNumber_cluster(filter,hash1);

    M_BIT_ARRAY_LENGTH_TYPE emptyLocationInBucket = 0;

    //Insert into the first bucket, to empty location.
    if(__is_bucket_empty_cluster(filter,bucket_1,&emptyLocationInBucket))
    {
...
//#endif
    ...
    //M_BIT_ARRAY_LENGTH_TYPE hash2 = __get_hash_2_cluster(finger_print,hash1);
    M_BIT_ARRAY_LENGTH_TYPE hash2 = __get_hash_2_cluster(finger_print,bucket_1);

    //This hash2 must be < finger_print array length.
    hash2 = hash2 % filter->__m_bit_arr_len_in_bytes;

    M_BIT_ARRAY_LENGTH_TYPE bucket_2 = __getBucketNumber_cluster(filter,hash2);

    if(__is_bucket_empty_cluster(filter,bucket_2,&emptyLocationInBucket))
    {
        //Fill this entry with the fingerprint.
        filter->__m_bit_finger_print_array[emptyLocationInBucket] = finger_print;
//#if PRINT_DEBUG
...
//#endif
        ...
        //firstIndex = __getFirstIndexOfTheBucket_cluster(filter,*(randomlyChosenBucket));
        //lastIndex = __getLastIndexOfTheBucket_cluster(filter,*(randomlyChosenBucket));
        ...
        for(cuckoo_kick_count = 1; cuckoo_kick_count <= __MAX_CUCKOO_KICKS; cuckoo_kick_count++)
        {
            ...
            actualElemsInBucket = __getLastIndexOfTheBucket_cluster(filter,*(randomlyChosenBucket)) - __getFirstIndexOfTheBucket_cluster(filter,*(randomlyChosenBucket)) ;
            ...
            randomlySelectedIndex = (randomNumber)+(__getFirstIndexOfTheBucket_cluster(filter,*(randomlyChosenBucket)));
            ...
            hash2 = __get_hash_2_cluster(finger_print,*(randomlyChosenBucket));
            //This hash2 must be < finger_print array length.
            hash2 = hash2 % filter->__m_bit_arr_len_in_bytes;
            ...
            bucket_2 = __getBucketNumber_cluster(filter,hash2);
...
*/
short
__check_or_delete_element_cluster(struct __cuckoo_filter* filter , const char* key, short __should_delete_element)
{
    ...
    if(__is_element_present_in_bucket_cluster(finger_print,bucket_1,filter,&locWhereElemIsPresent))
    {
    ...
    if(__is_element_present_in_bucket_cluster(finger_print,bucket_2,filter,&locWhereElemIsPresent))
    {
...
*/
short
__remove_element_cluster(struct __cuckoo_filter* filter , const char* key)
{
    serverLog(LL_NOTICE,"\"Removing the element\"");
    if(NULL == key)
        return 0;
    return __check_or_delete_element_cluster(filter,key,1);
}
...
*/
short
__remove_specified_filter_cluster(struct __cuckoo_filter* temp)
{
...
short
add_element_cluster(struct __cuckoo_filter* filter,const char* key)
{
    serverLog(LL_NOTICE,"Adding element to cuckoo filter");
    if(NULL == key)
        return 0;
    return __insert_element_cluster(filter,key);
}
...
/*
    ...
    Output :
        ...
        Returns 1 -- if the key is found in the cuckoo filter 'name'
*/
//unsigned short is_member_cluster(const char* name, const int key)
short
is_member_cluster(struct __cuckoo_filter* filter, const char* key)
{
    serverLog(LL_NOTICE,"Checking if element is member of filter.");
    if( NULL == key)
        return 0;

    return __check_or_delete_element_cluster(filter,key,0);
}
...
*/
short
delete_element_cluster(struct __cuckoo_filter* filter, const char* key)
{
    serverLog(LL_NOTICE,"Deleting element from filter");
    if(NULL == key)
        return 0;

    return __remove_element_cluster(filter,key);
}
...
void cuckooremoveclusterCommand(client *c) {
    ...
    */
    ...
    for (i = 2; i < c->argc; i++) {
        ...
        if(delete_element_cluster(filter,(c->argv[i])->ptr))
            addReply(c,shared.cone);
        else
            addReply(c,shared.czero);
    }
